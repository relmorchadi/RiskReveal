...
public class FacContractSpecification extends BaseSpecification<FacContract, FacContract> {
    ...
    public Specification<FacContract> getFilter(FacContract request) {
        return Specification
                ...
                .and(ofNullable(request.getRequestedByFirstName()).map(reauestedByFirstName -> AttributeContains(FacContract_.requestedByFirstName, reauestedByFirstName)).orElse(null))
                .and(ofNullable(request.getRequestedByLastName()).map(reauestedByLastName -> AttributeContains(FacContract_.requestedByLastName, reauestedByLastName)).orElse(null))
