...
public interface UserDashboardWidgetColumnsRepository extends JpaRepository<UserDashboardWidgetColumns,Long> {
    ...
    @Transactional
    @Modifying
    @Query(value = "exec dbo.update_Widget_table_width @columnId=:columnId,  @width=:width", nativeQuery = true)
    void updateDashboardWidgetColumnWidth(@Param("columnId") long columnId, @Param("width") int width);

    @Transactional
    @Modifying
    @Query(value = "exec dbo.usp_Reset_Sort @widgetId=:widgetId", nativeQuery = true)
    void resetSort(@Param("widgetId") long widgetId);

    @Transactional
    @Modifying
    @Query(value = "exec dbo.usp_Reset_Filter @widgetId=:widgetId", nativeQuery = true)
    void resetFilter(@Param("widgetId") long widgetId);

    @Transactional
    @Modifying
    @Query(value = "exec dbo.update_Widget_table_sort @columnId=:columnId,  @sort=:sort, @sorting=:sorting", nativeQuery = true)
    void updateDashboardWidgetColumnSort(@Param("columnId") long columnId, @Param("sort") int sort, @Param("sorting") String sorting);

    @Transactional
    @Modifying
    @Query(value = "exec dbo.update_Widget_table_filter @columnId=:columnId, @filter=:filter", nativeQuery = true)
    void updateDashboardWidgetColumnFilter(@Param("columnId") long columnId, @Param("filter") String filter);

    @Transactional
    @Modifying
    @Query(value = "exec dbo.update_Widget_table_visibleOrder @columnIdArray=:columnIdArray, @visibleArray=:visibleArray, @orderArray=:orderArray, @counter=:counter", nativeQuery = true)
