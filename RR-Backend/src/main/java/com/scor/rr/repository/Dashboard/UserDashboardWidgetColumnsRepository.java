...
public interface UserDashboardWidgetColumnsRepository extends JpaRepository<UserDashboardWidgetColumns,Long> {
    ...
    void updateDashboardWidgetColumnWidth(@Param("columnId") long columnId, @Param("width") Integer width);

    @Transactional
    @Query(value = "exec dbonew.update_Widget_table_sort @columnId=:columnId,  @sort=:sort, @sorting=:sorting", nativeQuery = true)
    void updateDashboardWidgetColumnSort(@Param("columnId") long columnId, @Param("sort") Integer sort, @Param("sorting") String sorting);
