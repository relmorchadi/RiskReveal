...
public class ELTToPLTConverter {
    ...
    public void batchConvert() {
        ...
        for (Map bundle : bundles) {
            ...
            Optional<RLAnalysis> rlAnalysisOpt = rlAnalysisRepository.findById(sourceResult.getRlAnalysisId().intValue());
            String analysisName = rlAnalysisOpt.map(RLAnalysis::getAnalysisName).orElse(null);
            BigInteger analysisId = rlAnalysisOpt.map(RLAnalysis::getAnalysisId).orElse(null);
