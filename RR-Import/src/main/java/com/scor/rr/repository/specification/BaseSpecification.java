...
import com.scor.rr.domain.riskLink.RLModelDataSource_;
...
public abstract class BaseSpecification<T, U> {
    ...
    protected Specification<T> AttributeEndsWith(SingularAttribute attribute, String value) {
        return (root, query, cb) -> {
            if(value == null) { return null; }
            return cb.like(cb.lower(root.get(attribute)), wildcard + StringUtils.lowerCase(value) );
        };
    }

    protected Specification<T> AttributeStartWith(SingularAttribute attribute, String value) {
        return (root, query, cb) -> {
            if(value == null) { return null; }
            return cb.like(cb.lower(root.get(attribute)), StringUtils.lowerCase(value) + wildcard);
        };
    }
    ...
    protected Specification<T> evaluateSpecialChars(SingularAttribute attribute, String value) {
        if(StringUtils.startsWith(value, "\"") && StringUtils.endsWith(value, "\"")){
            ...
            if (StringUtils.startsWith(value, "*")){
                ...
                return AttributeStartWith(attribute, StringUtils.remove(value, "*"));
            }
        }else {
            return AttributeContains(attribute, value);
        }
    }
