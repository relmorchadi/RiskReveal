...
public interface DefaultReturnPeriodRepository extends JpaRepository<DefaultReturnPeriodEntity, Long> {
    ...
    @Transactional
    @Query(value = "exec dbo.usp_ValidateRP @rp= :rp", nativeQuery = true)
    List<Map<String, Object>> validateRP(@Param("rp") Integer rp);

    @Transactional
    @Query(value = "exec dbo.usp_FindSinglePLTEpMetricsByPLTHeaderIdAndUserAndCurveType @userId=:userId, @PLTHeaderId=:pltHeaderId, @curveType=:curveType, @screen=:screen", nativeQuery = true)
    ...
    @Transactional
    @Query(value = "exec dbo.usp_FindEpMetricsByWorkspaceAndUserAndCurveType @userId=:userId, @workspaceContextCode=:workspaceContextCode, @uwYear=:uwYear, @type=:curveType, @screen=:screen", nativeQuery = true)
    ...
    @Transactional
    @Query(value = "exec dbo.usp_FindSingleEpMetricsByWorkspaceAndCurveTypeAndRP @workspaceContextCode=:workspaceContextCode, @uwYear=:uwYear, @curveType=:curveType, @rp=:rp", nativeQuery = true)
