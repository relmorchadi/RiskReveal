...
import {AuthState, HeaderState} from '../../../store/states';
...
import {JobManagerService} from "../../../service/jobManager.service";
...
export class TasksMenuItemComponent extends BaseContainer
    implements OnInit {
    ...
    initData() {
        ...
        this.jobManagerService.getAllJobs().subscribe((jobs: any) => {
            let myJobs = jobs.filter(row => row.submittedByUser == this.user);
            console.log(myJobs);
            this.savedTasksLocal = myJobs.map(row => ({
                ...row,
                append: false,
                selected: false,
            })).filter(row => row.status != 'SUCCEEDED' && row.status != 'FAILED');
            this.completedTasks = myJobs.map(row => ({
                ...row,
                append: false,
                selected: false,
            })).filter(row => row.status == 'SUCCEEDED' || row.status == 'FAILED');
            this.allTasks = this.savedTasksLocal;
            this.loading = false;
            this.detectChanges();
        });
    }
    ...
    deleteJob(id) {
        this.jobManagerService.deleteJob(id).subscribe((result: any) => {
            if (result) {
                this.savedTasksLocal = this.savedTasksLocal.filter(row => row.jobId != id);
                this.detectChanges();
            }
        });
        ;
    ...
    pauseJob(id) {
        this.jobManagerService.pauseJob(id).subscribe((result: any) => {
            if (result) {
                this.savedTasksLocal.find(row => row.jobId == id).status = 'PAUSED';
                this.detectChanges();
            }
        });
    }
